...
:Action(Gun)
{
  ...
  efficiencyBOMABCmd = new G4UIcmdWith3VectorAndUnit("/DetSys/gun/positionBOMABVolume",this);
  efficiencyBOMABCmd->SetGuidance("emit over BOMAB volume.");
  efficiencyBOMABCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
}
...
void PrimaryGeneratorMessenger::SetNewValue( G4UIcommand* command, G4String newValue )
{
  ...
  if( command == efficiencyBOMABCmd ) {
    Action->SetEfficiencyBOMAB(efficiencyBOMABCmd->GetNew3VectorValue(newValue));
  }
}
