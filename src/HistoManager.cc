...
//		detString[i] = G4intToG4String(i);
  ...
  if(WRITEEDEPHISTOS) {
    filename  = "germanium_cylinder_crystal_edep";
    ...
    filename  = "germanium_cylinder_crystal_edep_sum";
    title     = "Edep in crystal (keV)";
    nbins     = EDEPNBINS;
    xmin      = EDEPXMIN;
    xmax      = EDEPXMAX;
    ...
    for (G4int i=0; i < MAXNUMDET; i++) {
        ...
        filename  = "germanium_cylinder_crystal_edep_det" + detString;
        title     = "Edep in crystal (keV)";
        nbins     = EDEPNBINS;
        xmin      = EDEPXMIN;
        xmax      = EDEPXMAX;
  ...
  if(WRITEEKINHISTOS) {
    for (G4int i=0; i < MAXNUMDET; i++) {
        ...
        filename  = "germanium_cylinder_crystal_ekin_det" + detString;
        title     = "EKin in crystal (keV)";
        nbins     = EDEPNBINS;
        xmin      = EDEPXMIN;
        xmax      = EDEPXMAX;
        MakeHisto(analysisManager, filename,  title, xmin, xmax, nbins);
  ...
  if(stepTrackerBool) {
      ...
      fNtColId[0] = analysisManager->CreateNtupleDColumn("eventNum");
      fNtColId[1] = analysisManager->CreateNtupleDColumn("stepNum");
      fNtColId[2] = analysisManager->CreateNtupleDColumn("cryNum");
      fNtColId[3] = analysisManager->CreateNtupleDColumn("detNum");
      fNtColId[4] = analysisManager->CreateNtupleDColumn("edep_keV");
      fNtColId[5] = analysisManager->CreateNtupleDColumn("x_mm");
      fNtColId[6] = analysisManager->CreateNtupleDColumn("y_mm");
      fNtColId[7] = analysisManager->CreateNtupleDColumn("z_mm");
      fNtColId[8] = analysisManager->CreateNtupleDColumn("time_sec");
      fNtColId[9] = analysisManager->CreateNtupleDColumn("empty");
